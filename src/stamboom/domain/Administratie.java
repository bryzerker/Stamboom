...
public class Administratie {
    ...
    public Persoon addPersoon(Geslacht geslacht, String[] vnamen, String anaam,
        ...
        //todo opgave 1 gedaan
        Persoon returnPersoon = new Persoon(nextPersNr,vnamen, anaam,tvoegsel, gebdat, gebplaats,geslacht,ouderlijkGezin );
        
        for(Persoon p: personen)
        {
            if(returnPersoon.getNaam().equals(p.getNaam()))
            {
                return null;
            }
        }
        personen.add(returnPersoon);
        nextPersNr++;
    ...
    public Gezin addHuwelijk(Persoon ouder1, Persoon ouder2, Calendar huwdatum) {
        ...
        Gezin returnGezin = new Gezin(nextGezinsNr, ouder1, ouder2);
        gezinnen.add(returnGezin);
        nextGezinsNr++;
    ...
    public ArrayList<Persoon> getPersonenMetAchternaam(String achternaam) {
        //todo opgave 1 gedaan
        ArrayList<Persoon> resultaatPersonen = new ArrayList<>();
        for(Persoon p: personen)
        {
            if(p.getAchternaam().equals(achternaam))
            {
                resultaatPersonen.add(p);
            }
        }
        return resultaatPersonen;
    }
    ...
    public Persoon getPersoon(String[] vnamen, String anaam, String tvoegsel,
        ...
        for(Persoon p: personen)
        {
            if(p.getVoornamen().equals(Arrays.toString(vnamen))
    ...
    /**
     *
     * @return de geregistreerde gezinnen gedaan
     */
    public List<Gezin> getGezinnen() {
       List resultaatGezinnen = new ArrayList();
       for(Gezin g: gezinnen)
       {
           resultaatGezinnen.add(g);
       }
       return resultaatGezinnen;
    }
